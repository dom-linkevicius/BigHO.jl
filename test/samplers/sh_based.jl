...
@testset "Hyperband rung-level failure handling (shrinkage/abandonment)" begin
    ...
    # R=9, η=3, r_min=1 -> bracket 1 has capacities rung1=9, rung2=3, rung3=1 (see _capacity).
    ...
    @test BigHO._bracket_decision(s, 1, runs) == BigHO._promote(1, 1, 8) # best of the 2 completed (id=8, value 8.0 < id=9's 9.0)
    ...
    push!(runs, BigHO.RunEntry(10, (r=3, a=8), [7.5 / 9], Dict{Symbol,Any}(:rung => 2, :bracket_k => 1)))
    @test BigHO._bracket_decision(s, 1, runs) == BigHO._promote(1, 1, 9)
    push!(runs, BigHO.RunEntry(11, (r=3, a=9), [8.5 / 9], Dict{Symbol,Any}(:rung => 2, :bracket_k => 1)))
    @test BigHO._bracket_decision(s, 1, runs) == BigHO._wait() # both Pending now, not stuck asking for a nonexistent 3rd
    ...
    @test BigHO._bracket_decision(s, 1, wiped) == BigHO._draw(2, 1)
...
@testset "ASHA rung-level failure handling (bracket-stall/rung-failure warnings)" begin
    ...
    @test count(l -> l.level == Logging.Warn && occursin("bracket 1 stalled at 9/13", l.message), logs2) == 1
    @test count(l -> l.level == Logging.Warn && occursin("rung 1 of bracket 1 completed with at least one failed trial", l.message), logs2) == 1
    @test BigHO._bracket_decision(s, 1, wiped) == BigHO._draw(2, 1) # moves on to bracket 2, doesn't block forever
