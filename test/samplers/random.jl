...
@testset "Optimization correctness and efficiency" begin
    ...
    @test true_min == 0
    ho_exact = Hyperoptimizer(p -> f_disc(p.x, p.y), (x=Continuous(1, 100, 1), y=Continuous(1, 100, 1)); n=15000)
    run!(ho_exact)
    ...
    ho_small = Hyperoptimizer(p -> f_disc(p.x, p.y), (x=Continuous(1, 100, 1), y=Continuous(1, 100, 1)); n=10)
    run!(ho_small)
    ...
    target = Float64(pi)
    ho_cont = Hyperoptimizer(p -> (p.x - target)^2, (x=Continuous(-10, 10, 0.01),); n=3000)
    run!(ho_cont)
...
@testset "Non-numerics" begin
    ...
    hor = Hyperoptimizer(p -> p.g(p.x), (g=Nominal([sin, exp, identity]), x=Continuous(0, 1, 1 / 99)); n=100)
    run!(hor)
    show(devnull, hor)
    @test minimum(hor) < ℯ
    @test length(history(hor)) == 100
