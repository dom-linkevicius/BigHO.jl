...
mutable struct _HBTrial
    params::Vector{Any} # this trial's hyperparameter values (excluding r), fixed across promotions
    rung::Int           # current (1-based) rung index within its bracket
end

mutable struct _HBState
    R::Int
    r_min::Int
    η::Int
    bracket::_HBBracket
    trials::Dict{Int,_HBTrial}
...
function _new_bracket(k::Int, R::Int, r_min::Int, η::Int)
    ...
    return _HBBracket(k, resources, capacities, zeros(Int, k), [Tuple{Int,Float64}[] for _ in 1:k], [Set{Int}() for _ in 1:k])
end
...
function _find_promotion(bracket::_HBBracket, η::Int, ready)
...
_asha_ready(bracket::_HBBracket, i::Int) = !isempty(bracket.told[i])
# Hyperband: only once every trial dispatched at that rung has been told a result.
_hyperband_ready(bracket::_HBBracket, i::Int) = length(bracket.told[i]) >= bracket.capacities[i]

function _promote!(state::_HBState, id::Int, rung::Int, promoted_id::Int)
    bracket = state.bracket
    push!(bracket.promoted[rung], promoted_id)
    new_rung = rung + 1
    trial = state.trials[promoted_id]
    state.trials[id] = _HBTrial(trial.params, new_rung)
    bracket.dispatched[new_rung] += 1
    state.pending_promoted_id = promoted_id
    return vcat(bracket.resources[new_rung], trial.params)
end

function _draw_new!(state::_HBState, id::Int, ctx::AskContext)
    bracket = state.bracket
    raw_params = [rand(d) for d in ctx.candidates[2:end]] # skip the reserved :r slot (position 1)
    state.trials[id] = _HBTrial(raw_params, 1)
    bracket.dispatched[1] += 1
    state.pending_promoted_id = nothing
...
"""
struct Hyperband <: Sampler
    state::_HBState
end
Hyperband(; R::Int, η::Int=3, r_min::Int=1) = Hyperband(_HBState(R, r_min, η, _new_bracket(_smax(R, r_min, η) + 1, R, r_min, η), Dict{Int,_HBTrial}(), nothing))
...
ASHA(; R::Int, η::Int=3, r_min::Int=1) = ASHA(_HBState(R, r_min, η, _new_bracket(_smax(R, r_min, η) + 1, R, r_min, η), Dict{Int,_HBTrial}(), nothing))

init(s::Union{Hyperband,ASHA}, ctx::AskContext) = s

function (s::Hyperband)(ctx::AskContext)
    state = s.state
    id = ctx.n_asked + 1
    bracket = state.bracket
    promotion = _find_promotion(bracket, state.η, _hyperband_ready)
    if promotion === nothing && all(i -> length(bracket.told[i]) >= bracket.capacities[i], 1:bracket.k)
        state.bracket = _new_bracket(_next_bracket_k(bracket.k, _smax(state.R, state.r_min, state.η)), state.R, state.r_min, state.η)
        bracket = state.bracket
        promotion = _find_promotion(bracket, state.η, _hyperband_ready) # a fresh bracket is never itself resolved
    end
    promotion !== nothing && return _promote!(state, id, promotion...)
    return _draw_new!(state, id, ctx)
end

function (s::ASHA)(ctx::AskContext)
    state = s.state
    id = ctx.n_asked + 1
    promotion = _find_promotion(state.bracket, state.η, _asha_ready)
    promotion !== nothing && return _promote!(state, id, promotion...)
    return _draw_new!(state, id, ctx)
end
...
function exhausted(s::Hyperband, ho)
    state = s.state
    bracket = state.bracket
    _find_promotion(bracket, state.η, _hyperband_ready) !== nothing && return false
...
function on_tell!(s::Union{Hyperband,ASHA}, entry)
    entry.status === Completed || return nothing # excluded from ranking via status, never a NaN placeholder
    trial = get(s.state.trials, entry.id, nothing)
    trial === nothing && return nothing
    push!(s.state.bracket.told[trial.rung], (entry.id, entry.value))
    return nothing
end

function create_run_entry(s::Union{Hyperband,ASHA}, ho, id, params)
    promoted_id = s.state.pending_promoted_id
    s.state.pending_promoted_id = nothing
    promoted_id === nothing && return RunEntry(id, params)
    return RunEntry(id, params; pre_artefact=ho.runs[promoted_id].post_artefact)
end
